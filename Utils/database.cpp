...
bool Database::clearTaskData(const QString &code,QString &msg){
    ...
    QString sql = QString("DELETE FROM '%1' ").arg(code);
    if(!query.exec(sql)){
        success = false;
        msg = QString("清空数据表失败:%1").arg(query.lastError().text());
        QLOG_INFO()<<"Database::clearTaskData Error:"<<query.lastError()<<","<<sql;
    }

    if(success){
        sql = QString("UPDATE sqlite_sequence SET seq = 0 WHERE name = '%1'; ").arg(code);
        if(!query.exec(sql)){
            success = false;
            msg = QString("重置自增序号失败:%1").arg(query.lastError().text());
            QLOG_INFO()<<"Database::clearTaskData Error:"<<query.lastError()<<","<<sql;
        }
    }

    if(success){
        sql = QString("DELETE FROM '%1' where tbName='%2' ").arg(TABLE_TASK_DATA).arg(code);
        if(!query.exec(sql)){
            success = false;
            msg = QString("清理任务运行记录失败:%1").arg(query.lastError().text());
            QLOG_INFO()<<"Database::clearTaskData Error:"<<query.lastError()<<","<<sql;
        }
    }
