...
//                file.dir().remove(file.fileName());
    ...
    // 打开“更新说明”汇总HTML（从资源释放到本地数据目录后用系统浏览器打开）
    static void openUpdateSummary(){
        QString htmlPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/update-2.0-summary.html";
        QDir dir(QFileInfo(htmlPath).absolutePath());
        if(!dir.exists()){
            dir.mkpath(".");
        }
