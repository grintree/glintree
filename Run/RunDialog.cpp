    ...
    if(mThreadPipline->jobCommandState){
        ...
        if(np){
            ...
            if(MCONSTANT_FLOW_MENU_TYPE_OpenWebBtn==menuType){
             ...
             }else if(MCONSTANT_FLOW_MENU_TYPE_CaptchaBtn==menuType){
                startCaptcha(static_cast<MFlowStepParamsCaptcha *>(np));
             }else if(MCONSTANT_FLOW_MENU_TYPE_SwitchBtn==menuType){
                startSwitch(static_cast<MFlowStepParamsSwitch *>(np));
             ...
             }else if(MCONSTANT_FLOW_MENU_TYPE_JudgeBtn==menuType){
                startJudge(static_cast<MFlowStepParamsIf *>(np));
             }else if(MCONSTANT_FLOW_MENU_TYPE_EndLoopBtn==menuType){
                startEndLoop(static_cast<MFlowStepParamsEndLoop *>(np));
             }else if(MCONSTANT_FLOW_MENU_TYPE_EndBtn==menuType){
                startEnd(static_cast<MFlowStepParamsEnd *>(np));
             }else{
...
void RunDialog::startOpenWeb(MFlowStepParamsOpenWeb *params){
     ...
     QTimer::singleShot(WAIT_SLEEP_UNIT*params->waitSleep,this,[this,url,params](){
         ...
         // 打开网页前清理浏览器缓存
         if(params->isClearCacheBeforeOpenWeb){
             mWebViewManager->profile()->clearHttpCache();
             QLOG_INFO()<<"RunDialog::startOpenWeb 打开网页前已清理浏览器缓存";
         }

         // 自定义Cookie
         if(params->isCustomCookie && !params->cookie.trimmed().isEmpty()){
             QWebEngineCookieStore *cookieStore = mWebViewManager->profile()->cookieStore();
             QStringList pairs = params->cookie.split(";",Qt::SkipEmptyParts);
             for (int i = 0; i < pairs.size(); ++i) {
                 QString pair = pairs.at(i).trimmed();
                 int eq = pair.indexOf('=');
                 if(eq > 0){
                     QNetworkCookie ck(pair.left(eq).trimmed().toUtf8(),
                                       pair.mid(eq + 1).trimmed().toUtf8());
                     ck.setDomain(qurl.host());
                     ck.setPath("/");
                     cookieStore->setCookie(ck,qurl);
                 }
             }
             QLOG_INFO()<<"RunDialog::startOpenWeb 已设置自定义Cookie";
         }

         mWebViewManager->pageIsFinished = false;
         mWebViewManager->getCurrentWebView()->load(request);

         // 加载超时自动重试（isAutoTry 开启时最多重试3次）
         int *tryCount = new int(0);
         int *ticks = new int(0);
         int maxTry = params->isAutoTry ? 3 : 1;
         int timeoutTicks = qMax(1, params->overtime * 20);// overtime单位秒，每50ms检测一次
         QTimer *timer = new QTimer();
         connect(timer,&QTimer::timeout,this,[this,timer,params,tryCount,ticks,maxTry,timeoutTicks](){
            ...
            if(*ticks >= timeoutTicks){
                *ticks = 0;
                ++(*tryCount);
                if(*tryCount <= maxTry){
                    QLOG_INFO()<<"RunDialog::startOpenWeb 加载超时，自动重试第 "<<*tryCount<<" 次 url="<<params->url;
                    mWebViewManager->pageIsFinished = false;
                    mWebViewManager->getCurrentWebView()->reload();
                }else{
                    timer->stop();
                    delete timer;
                    delete tryCount;
...
void RunDialog::startClickEle(MFlowStepParamsClickEle *params){
    ...
    if(params->eleXpath=="" && params->eleXpath==""){
        ...
        QTimer::singleShot(WAIT_SLEEP_UNIT*params->waitSleep,this,[this,script,params](){
             mWebViewManager->getCurrentWebView()->page()->runJavaScript(script,[this,params,script](const QVariant &v) {
                  QString feedback = v.toString();
                  QLOG_INFO() << QString("Run.cpp startClickEle() feedback=%1").arg(feedback);

                  if(feedback.startsWith("success")){
                      if(params->isAjax && params->ajaxOvertime > 0){
                          // Ajax加载：点击后等待指定时长再继续下一步
                          QLOG_INFO()<<"RunDialog::startClickEle Ajax加载，等待 "<<params->ajaxOvertime<<" 秒";
                          QTimer::singleShot(params->ajaxOvertime * 1000,this,[this,params,feedback,script](){
                              startClickEleRoll(params,feedback,script);
                          });
                      }else{
                          startClickEleRoll(params,feedback,script);
                      }
...
void RunDialog::startClickEleRoll(MFlowStepParamsClickEle *params, QString feedback, QString clickScript){
      ...
      if("dropdown"==p1) {
         ...
         if(params->isRoll){
          ...
          QTimer::singleShot(wait,this,[this,params,clickNext,clickScript](){

              // 页面加速：非Ajax页面不等待整页加载完成
              if(params->isPageSpeed){
                  if(params->isRoll){
                      mWebViewManager->getCurrentWebView()->page()->runJavaScript(QString("startRoll('%1','%2','%3')").arg(params->rollCount).arg(params->rollInterval).arg(params->rollTypeIndex),
              ...
              int *retry = new int(0);
              QTimer *timer = new QTimer();
              connect(timer,&QTimer::timeout,this,[this,timer,params,clickNext,ticks,retry,clickScript](){
                 if(mWebViewManager->pageIsFinished){
                     timer->stop();
                     delete timer;
                     delete ticks;
                     ...
                     auto continueAfterLoad = [this,params,clickNext](){
                         if(params->isRoll){
                             mWebViewManager->getCurrentWebView()->page()->runJavaScript(QString("startRoll('%1','%2','%3')").arg(params->rollCount).arg(params->rollInterval).arg(params->rollTypeIndex),
                                                                                    [this,params,clickNext](const QVariant &v) {
                                   QString rf = v.toString();
                     ...
                     // 定位锚点：页面加载完成后滚动到指定元素
                     if(params->isAnchor && !params->anchorId.isEmpty()){
                         mWebViewManager->getCurrentWebView()->page()->runJavaScript(
                             QString("startAnchor('%1')").arg(params->anchorId),
                             [continueAfterLoad](const QVariant &){
                                 continueAfterLoad();
                             });
                     }else{
                         continueAfterLoad();
                 ...
                 // 自动重试：页面长时间没有完成加载时，重新执行点击脚本
                 ++(*ticks);
                 if(*ticks >= 100){// 5秒
                     *ticks = 0;
                     ++(*retry);
                     if(params->isAutoTry && !clickScript.isEmpty() && *retry <= 3){
                         QLOG_INFO()<<"RunDialog::startClickEleRoll 页面无变化，自动重试点击第 "<<*retry<<" 次";
                         mWebViewManager->pageIsFinished = false;
                         mWebViewManager->getCurrentWebView()->page()->runJavaScript(clickScript,[](const QVariant &){});
                     }else{
                         timer->stop();
                         delete timer;
                         delete ticks;
...
void RunDialog::startCaptcha(MFlowStepParamsCaptcha *params){
    ...
    if(params->carryParent!=nullptr){//当前步骤存在父窗体
        if(params->carryParent->menuType==MCONSTANT_FLOW_MENU_TYPE_LoopBtn){
            ...
            if(params->carryStepLocation==MFlowStepParams::LocationType::Last){
                if(ploop->loopTypeName==MCONSTANT_RADIO_SINGLE){
                  sendStopJobCommand(RunMessage::MessageType::ERROR,"验证码不能作单个元素循环的最后一步，规则错误！");
                  return ;
                }
            }
         }else{
            sendStopJobCommand(RunMessage::MessageType::WARNING,"（05x）暂不支持if窗体！");
            return ;
        }
   }

    QTimer::singleShot(WAIT_SLEEP_UNIT*params->waitSleep,this,[this,params,iloopTypeName,iloopTypeValue,inum](){
        // 从网页上提取验证码图片
        mWebViewManager->getCurrentWebView()->page()->runJavaScript(
                    QString("startCaptchaImg('%1')").arg(params->eleXpath),
                    [this,params,iloopTypeName,iloopTypeValue,inum](const QVariant &v) {
            QString feedback = v.toString();
            QByteArray imageData;
            QString format = "PNG";

            if(feedback.startsWith("success:")){
                QString data = feedback.mid(8);
                if(data.startsWith("data:image/")){
                    int comma = data.indexOf(',');
                    format = data.mid(11, comma - 11).toUpper();
                    if(format.contains(';')){
                        format = format.left(format.indexOf(';'));
                    }
                    imageData = QByteArray::fromBase64(data.mid(comma + 1).toUtf8());
                }else{
                    imageData = data.toUtf8();
                    format.clear();
                }
            }else if(feedback.startsWith("url:")){
                // 跨域图片无法截取时，直接下载图片地址
                QString url = feedback.mid(4);
                ...
                sendStopJobCommand(RunMessage::MessageType::ERROR,"获取验证码图片失败，请确认规则后重试！");
                return ;
            }

            RunCaptchaDialog dlg(this,imageData,format);
            if(dlg.exec()!=QDialog::Accepted){
                sendStopJobCommand(RunMessage::MessageType::WARNING,"用户取消了验证码输入");
                return ;
            }
            if(dlg.isSkipped()){
                // 跳过验证码，继续执行下一步
            ...
            QString text = dlg.inputText();
            if(text.isEmpty()){
                sendStopJobCommand(RunMessage::MessageType::ERROR,"验证码不能为空！");
                return ;
            }

            QString script = QString("startInput('%1','%2','%3','%4','%5','%6')").
                    arg(iloopTypeName).arg(iloopTypeValue).arg(inum).
                    arg(params->inputXpath).arg(text).arg(params->isLoop);
            mWebViewManager->getCurrentWebView()->page()->runJavaScript(script,[this,params](const QVariant &rv) {
                QString rf = rv.toString();
                if(rf.startsWith("success")){
                    endCaptcha(params);
                }else{
                    sendStopJobCommand(RunMessage::MessageType::ERROR,"验证码输入失败，请确认规则后重试！");
                }
            });
        });
    });
}
void RunDialog::endCaptcha(MFlowStepParamsCaptcha *params){
    QString lastStepID = params->stepID;
...
void RunDialog::startSwitch(MFlowStepParamsSwitch *params){
    ...
    if(params->carryParent!=nullptr){//当前步骤存在父窗体
        if(params->carryParent->menuType==MCONSTANT_FLOW_MENU_TYPE_LoopBtn){
            ...
            if(params->carryStepLocation==MFlowStepParams::LocationType::Last){
                if(ploop->loopTypeName==MCONSTANT_RADIO_SINGLE){
                  sendStopJobCommand(RunMessage::MessageType::ERROR,"切换下拉选项不能作单个元素循环的最后一步，规则错误！");
                  return ;
                }
            }
         }else{
            sendStopJobCommand(RunMessage::MessageType::WARNING,"（06x）暂不支持if窗体！");
            return ;
        }
   }

    QString script = QString("startSwitch('%1','%2','%3','%4','%5','%6')").
            arg(iloopTypeName).arg(iloopTypeValue).arg(inum).
            arg(params->eleXpath).arg(params->switchValue).arg(params->isLoop);

    QTimer::singleShot(WAIT_SLEEP_UNIT*params->waitSleep,this,[this,script,params](){
        mWebViewManager->getCurrentWebView()->page()->runJavaScript(script,[this,params](const QVariant &v) {
            QString feedback = v.toString();
            QLOG_INFO() << QString("Run.cpp startSwitch() feedback=%1").arg(feedback);
            if(feedback.startsWith("success")){
                endSwitch(params);
            }else{
                sendStopJobCommand(RunMessage::MessageType::ERROR,"切换下拉选项失败，请确认规则后重试！");
                return ;
            }
        });
    });
}
void RunDialog::endSwitch(MFlowStepParamsSwitch *params){
    QString lastStepID = params->stepID;
...
void RunDialog::startJudge(MFlowStepParamsIf *params){

    QString judgeType = params->judgeType.isEmpty() ? QString("element_exists") : params->judgeType;
    QString script = QString("startJudge('%1','%2','%3')").
            arg(judgeType).arg(params->eleXpath).arg(params->judgeValue);

    QTimer::singleShot(WAIT_SLEEP_UNIT*params->waitSleep,this,[this,script,params](){
        mWebViewManager->getCurrentWebView()->page()->runJavaScript(script,[this,params](const QVariant &v) {
            QString feedback = v.toString();
            QLOG_INFO() << QString("Run.cpp startJudge() feedback=%1").arg(feedback);
            if(feedback.startsWith("success:")){
                bool pass = (feedback.mid(8).trimmed()=="true");
                if(pass){
                    // 条件成立：进入判断窗体内部的第一步
                    startNextStep(params->stepID);
                }else{
                    // 条件不成立：跳过判断窗体内部的所有步骤
                    MFlowStepParams *after = mHelper->getNextStepAfterBlock(params);
                    if(after){
                        startNextStep(after->stepID);
                    }else{
                        sendStopJobCommand(RunMessage::MessageType::SUCCESS,"判断条件不成立，流程结束");
                    }
                }
            }else{
                sendStopJobCommand(RunMessage::MessageType::ERROR,"判断条件执行失败，请确认规则后重试！");
                return ;
            }
        });
    });
}
void RunDialog::startEndLoop(MFlowStepParamsEndLoop *params){
    QLOG_INFO() << "RunDialog startEndLoop()";
    if(params->carryParent!=nullptr && params->carryParent->menuType==MCONSTANT_FLOW_MENU_TYPE_LoopBtn){
        this->startLoopTail(params->carryParent->stepID,params->stepID);
    }else{
        this->startNextStep(params->stepID);
    }
}
void RunDialog::startEnd(MFlowStepParamsEnd *params){
    Q_UNUSED(params);
    QLOG_INFO() << "RunDialog startEnd() 流程执行结束";
    sendStopJobCommand(RunMessage::MessageType::SUCCESS,"流程执行结束");
}
