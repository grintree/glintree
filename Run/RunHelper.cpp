...
MFlowStepParams * RunHelper::getNextStepAfterBlock(MFlowStepParams *blockParams){
    ...
    int blockIndex = -1;
    for (int i = 0; i < stepCount; ++i) {
        if(mTaskDeserialize->steps[i]==blockParams){
            blockIndex = i;
            break;
        }
    }
    ...
    // 找到容器内的最后一步（按先序遍历，其后一步即为容器之后的第一步）
    int lastInnerIndex = blockIndex;
    for (int i = blockIndex + 1; i < stepCount; ++i) {
        MFlowStepParams *p = mTaskDeserialize->steps[i];
        bool inBlock = false;
        while (p != nullptr) {
            if(p->carryParent==blockParams){
                inBlock = true;
                break;
            }
            p = p->carryParent;
    ...
    int next = lastInnerIndex + 1;
    if(next < stepCount){
        return mTaskDeserialize->steps[next];
    }
    return nullptr;
}
