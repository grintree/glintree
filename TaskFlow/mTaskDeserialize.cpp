    ...
    for (int i = 0; i < programList.length(); ++i) {
        ...
        QString programBase64 = programList[i].mid(4);
         ...
         if(MCONSTANT_FLOW_MENU_TYPE_OpenWebBtn==menuType){
             ...
             MFlowStepParamsCaptcha *params = new MFlowStepParamsCaptcha(QString(QByteArray::fromBase64(programBase64.toUtf8()).data()));
             params->carryParent = parentParams;
             params->carryStepLocation =getStepLocationType(start,end,i);
             steps.append(params);
             ...
             MFlowStepParamsSwitch *params = new MFlowStepParamsSwitch(QString(QByteArray::fromBase64(programBase64.toUtf8()).data()));
             params->carryParent = parentParams;
             params->carryStepLocation =getStepLocationType(start,end,i);
             steps.append(params);
             ...
             appendIfParams(deep,getStepLocationType(start,end,i),programBase64,parentParams);
             ...
             MFlowStepParamsEndLoop *params = new MFlowStepParamsEndLoop(QString(QByteArray::fromBase64(programBase64.toUtf8()).data()));
             params->carryParent = parentParams;
             params->carryStepLocation =getStepLocationType(start,end,i);
             steps.append(params);

         }else if(MCONSTANT_FLOW_MENU_TYPE_EndBtn==menuType){
             MFlowStepParamsEnd *params = new MFlowStepParamsEnd(QString(QByteArray::fromBase64(programBase64.toUtf8()).data()));
             params->carryParent = parentParams;
             params->carryStepLocation =getStepLocationType(start,end,i);
             steps.append(params);
...
void MTaskDeserialize::appendIfParams(int deep,MFlowStepParams::LocationType stepLocation,const QString &programBase64, MFlowStepParams *parentParams){
    // programsBase64  翻译后 ifBase64[step1Base64,step2Base64,,,]

    QString program = QString(QByteArray::fromBase64(programBase64.toUtf8()).data());
    int subStart = program.indexOf(SEPARATOR_SUB_LEFT);
    int subEnd = program.indexOf(SEPARATOR_SUB_RIGHT);
    QString ifProgramBase64 = program.mid(0,subStart);
    // 判断窗体自身
    MFlowStepParamsIf *params = new MFlowStepParamsIf(QString(QByteArray::fromBase64(ifProgramBase64.toUtf8()).data()));
    params->carryParent = parentParams;
    params->carryStepLocation = stepLocation;
    steps.append(params);
    ...
    // 判断窗体内部的 step
    QString subProgram = program.mid(subStart+1,subEnd-subStart-1);
    appendParams(deep,subProgram,params);
}
