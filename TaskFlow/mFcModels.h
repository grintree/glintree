...
const QString m_stylesheet_QWidget_flowleftwidget_blue_normal = "QWidget {background-color: rgb(10,132,255); \
        border-radius:5px; padding:2px 4px;} QWidget:click{background-color: rgb(0,106,224);}";
// 修饰流程步骤窗体按下状态下的样式 偏蓝色
const QString m_stylesheet_QWidget_flowleftwidget_blue_press = "QWidget {background-color: rgb(0,106,224); border-radius:5px; padding:2px 4px;}";

const QString m_stylesheet_QLabel_title = ".QLabel{font-family:Microsoft YaHei;font-size:16px;background-color: rgba(0,0,0,0);color:rgb(255,255,255);}";
...
public:
    ...
    MFlowStepParamsCaptcha(const QString &program);
public:
    QString eleXpath;//验证码图片元素xpath
    QString inputXpath;//验证码输入框xpath
    int waitSleep=0;//执行前等待时间 s
    bool isLoop = false;// 使用循环
    const QString getProgram();
};
...
public:
    ...
    QString waitEmergintEle;//或出现元素
    bool isLoop = false;// 使用循环
    QString switchValue;//要切换到的选项文本或value
    const QString getProgram();
};
//! 判断窗体
...
public:
    ...
    QString waitEmergintEle;//或出现元素
    bool isLoop = false;// 使用循环
    QString judgeType;//判断方式：element_exists 元素存在 / text_equal 文本等于 / text_contains 文本包含
    QString eleXpath;//判断元素xpath
    QString judgeValue;//期望值
...
public:
    ...
    MFlowStepParamsEndLoop(const QString &program);
    const QString getProgram();
};
//! 结束流程
...
public:
    MFlowStepParamsEnd();
    MFlowStepParamsEnd(const QString &program);
    const QString getProgram();
};
//! 步骤窗体的参数
class MFlowStepData{
