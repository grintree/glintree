...
void MFlowRightClickEleWidget::onSave(bool clicked){
    ...
    m_params->isAjax = isAjaxCheck->checkState()==Qt::CheckState::Checked?true:false;
    m_params->ajaxOvertime = ajaxOvertimeSpin->value();
    m_params->isPageSpeed = isPageSpeedCheck->checkState()==Qt::CheckState::Checked?true:false;
    m_params->isAnchor = isAnchorCheck->checkState()==Qt::CheckState::Checked?true:false;
    m_params->anchorId = anchorIdLine->text().trimmed();
    m_params->isAutoTry = autoTryCheck->checkState()==Qt::CheckState::Checked?true:false;
