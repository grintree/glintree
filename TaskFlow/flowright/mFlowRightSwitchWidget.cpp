...
void MFlowRightSwitchWidget::onSave(bool clicked){
    ...
    m_params->switchValue = switchValueLine->text().trimmed();
...
void MFlowRightSwitchWidget::initSenierOptionUI(){
    ...
    ++optionBox->rowStart;
      QLabel *switchValueLabel = new QLabel(optionBox->gWidget);
      switchValueLabel->setText(tr("切换选项值"));
      switchValueLine = new QLineEdit(optionBox->gWidget);
      switchValueLine->setStyleSheet(m_stylesheet_QLineEdit);
      switchValueLine->setText(this->m_params->switchValue);
      switchValueLine->setPlaceholderText("选项文本或 value");
      optionBox->gLayout->addWidget(switchValueLabel,optionBox->rowStart,0);
      optionBox->gLayout->addWidget(switchValueLine,optionBox->rowStart,1);
