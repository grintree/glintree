...
#include "flowright/mFlowRightCaptchaWidget.h"
...
PageCustomTaskFlow::PageCustomTaskFlow(MTask *task,QWidget *parent) : QWidget(parent),m_task(task){
    ...
    line->setStyleSheet(".QWidget{background-color:rgb(229,229,234);}");
...
void PageCustomTaskFlow::initMenuUi(){
    ...
    menuWidget->setStyleSheet("QWidget{background-color:rgb(245,245,247)}");
    ...
    contraolsWidget->setFixedHeight(330); // 30 * 11 一个菜单项的高度30
    QStringList controls = QStringList()<<MCONSTANT_FLOW_MENU_TYPE_OpenWebBtn<<MCONSTANT_FLOW_MENU_TYPE_ClickEleBtn
          ...
          <<MCONSTANT_FLOW_MENU_TYPE_CaptchaBtn<<MCONSTANT_FLOW_MENU_TYPE_SwitchBtn
          <<MCONSTANT_FLOW_MENU_TYPE_JudgeBtn<<MCONSTANT_FLOW_MENU_TYPE_LoopBtn
          <<MCONSTANT_FLOW_MENU_TYPE_MouseBtn<<MCONSTANT_FLOW_MENU_TYPE_EndLoopBtn
          <<MCONSTANT_FLOW_MENU_TYPE_EndBtn;
...
void PageCustomTaskFlow::onNotificationFromFlowControl(MFlowStepData* data){
    ...
    //        // 断开信号的总结：https://www.jianshu.com/p/986851c507da
            ...
            if(MCONSTANT_FLOW_MENU_TYPE_OpenWebBtn==data->control->menuType){
                ...
                r->setAttribute(Qt::WA_DeleteOnClose);
                rightScrollWidgetLayout->addWidget(r);
            }else if(MCONSTANT_FLOW_MENU_TYPE_CaptchaBtn==data->control->menuType){

                MFlowRightCaptchaWidget *r =new MFlowRightCaptchaWidget(this,data);
                r->setAttribute(Qt::WA_DeleteOnClose);
                rightScrollWidgetLayout->addWidget(r);
