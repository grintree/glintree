...
{
    QCoreApplication::setOrganizationName("mushi");
    QCoreApplication::setOrganizationDomain("grintree.mushi.com");
    QCoreApplication::setApplicationName("Glintree");
...
#if defined(Q_OS_WIN)
    // 根据屏幕分辨率和DPI计算UI缩放系数，适配不同电脑屏幕（1080P为基准，1.0~1.75）
    int screenW = GetSystemMetrics(SM_CXSCREEN);
    UINT dpi = GetDpiForSystem();
    double logicalW = dpi > 0 ? double(screenW) * 96.0 / double(dpi) : double(screenW);
    double uiScale = logicalW / 1920.0;
    if(uiScale < 1.0){
        uiScale = 1.0;
    }
    if(uiScale > 1.75){
    ...
    // 应用图标统一使用项目根目录的 logo.ico
    a.setWindowIcon(QIcon(QStringLiteral(":/logo.ico")));
    QFile qss(":/res/qss/style.qss");
    qss.open(QFile::ReadOnly);
    a.setStyleSheet(qss.readAll());
    qss.close();

    QSharedMemory shared("Glintree");
    if(shared.attach()){
    ...
    QString vInfo = QString("Glintree V%1,qwebengine V%2 chromium V%3 chromiumSP V%4").
